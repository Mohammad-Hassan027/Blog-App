...
import useBlogStore from "../../store/useBlogStore";
import { usePosts } from "../../hooks/blogHooks"; // Import the custom hook

// Define the page limit here, matching the previous logic or API default
const POSTS_PER_PAGE = 6;

function AllPostsPage() {
  const [searchTerm, setSearchTerm] = useState("");

  // Get client-side pagination state and setter from Zustand
  const blogListPage = useBlogStore((s) => s.blogListPage);
  const setBlogListPage = useBlogStore((s) => s.setBlogListPage);

  // Use TanStack Query hook to fetch data based on the current page state
  const { data, isLoading, isError, error } = usePosts(
    blogListPage,
    POSTS_PER_PAGE
  );

  // Extract data safely from the query result
  const blogs = data?.blogs || [];
  const currentPage = data?.currentPage || 1;
  const totalPages = data?.totalPages || 1;

  // Search logic remains local
  const filteredBlogs = [...blogs].filter(
    (blog) =>
      blog.title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      blog.author?.toLowerCase().includes(searchTerm.toLowerCase())
  ...
  // NOTE: The useEffect for initial fetch is removed.
  // TanStack Query handles the initial fetch automatically.

  // Use isLoading and isError from TanStack Query
  const displayError = isError ? error.message : null;

  const handlePageChange = (pageNumber) => {
    // Only update the client-side state. TanStack Query will automatically
    // trigger a re-fetch because the 'blogListPage' used in usePosts dependency changed.
